...
def p2pkh_address():
    return _(r'(1|n|m)[1-9A-HJ-NP-Za-km-z]{33}')
...
def p2sh_address():
    return [_(r'3[1-9A-HJ-NP-Za-km-z]{33}'), _(r'2[1-9A-HJ-NP-Za-km-z]{34}')]


# Bech32 or Bech32m encoded
# All of them have 42 characters
def p2wpkh_address():
    return _(r'(bc|tb)1[01-9ac-hj-np-z]{39}')


# Bech32 or Bech32m encoded
# All of them have 62 characters
def p2wpsh_address():
    return _(r'(bc|tb)1[01-9ac-hj-np-z]{59}')
...
def segwit_address():
    return [p2wpkh_address, p2wpsh_address, p2tr_address]


def addr_spec():
    return [p2pkh_address, p2sh_address, segwit_address]


# -------------------- START RULES FOR KEY ------------------------------------

def key_fingerprint():
    return _(r'[0-9a-fA-F]{8}')


def key_origin():
    return '[', key_fingerprint, ZeroOrMore('/', num, Optional(["'", 'h'])), ']'


# Constrained by KeyPublicHexUncompressedVisitor
def key_public_hex_uncompressed():
    return ['04'], _(r'[0-9a-fA-F]{130}')


# Constrained by KeyPublicHexXOnlyVisitor
def key_public_hex_xonly():
    return _(r'[0-9a-fA-F]{64}')


def key_public_hex_compressed():
    return ['02', '03'], key_public_hex_xonly


def key_public_hex():
    return [key_public_hex_compressed, key_public_hex_uncompressed]


# Base58
# Constrained by KeyPrivateWifVisitor
def key_private_wif():
    return _(r"[1-9A-HJ-NP-Za-km-z]{51,52}")


# Base58. Total length must be 111
# Constrained by KeyStartB58Visitor
def key_start_b58():
    return ['xpub', 'xpriv', 'tpub', 'tpriv'], _(r"[1-9A-HJ-NP-Za-km-z]{107}")


def key_simple_path():
    return ZeroOrMore('/', num, Optional(["'", 'h']))


def key_multibranch():
    return '<', num, ZeroOrMore(';', num), '>'
...
def key_extended():
    return key_start_b58, key_extended_path
...
def key():
    return Optional(key_origin), [key_public_hex, key_extended, key_private_wif]
...
def script_tr():
    return tr([key, key_public_hex_xonly], Optional(',', tree))
...
def script_key():
    return [pk(key), pkh(key)]


def key_sequence():
    return key, OneOrMore(',', key)


def key_sequence_xonly():
    return [key, key_public_hex_xonly], OneOrMore(',', [key, key_public_hex_xonly])
...
def subscript():
    return [script_key, multi_key]


def script():
    return [
        sh(subscript), sh(wsh(subscript)), sh(wpkh(key_no_uncompressed)), wsh(subscript), script_key, multi_key,
        wpkh(key_no_uncompressed), combo(key), script_tr, addr(addr_spec), raw(hex_str), rawtr(key)
    ]
...
def checksum():
    return _(r'[0-9a-zA-Z]{8}')


def descriptor():
    return script, Optional('#', checksum), EOF
...
def ecpoint():
    return field_elem, ',', field_elem


def curve_key_point():
    return [ecpoint, field_elem], EOF
...
def privkey_hex_inwif():
    return ['80', 'ef'], privkey_hex, Optional('01'), EOF
...
def compressed_key_data():
    return [key_public_hex_compressed, ('00', privkey_hex)]
