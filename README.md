# Bitcoin descriptor specification
...
Since we need to use Python syntax, some particular notation in the grammar rules might be non-trivial:
 ...
 - Some keywords are function names, others are function calls. For example, `addr_spec` is always used as a function object, whereas `addr` is a function call, that has the purpose of a macro. `addr` itself is not a grammar rule, but a helper to construct rules.
...
# Output descriptor specification

A `descriptor` is an ASCII string that contains a `script`. The descriptor can
be followed by "#" and a checksum:
- `descriptor` = `script` \['#' `checksum`\]

A `checksum` is hex-string of 8 characters.

A `script` is of 3 types:
- `topscript`: scripts that can only be placed at top level. They can't be inside another script.
- `subscript`: scripts that can be placed at the top level, or in particular other scripts.
- `taproot_script`: scripts that start with "tr(". Can be placed only at the top level.

The reason `taproot_script` is differentiated from `topscript` is because its
contents differ significantly from the contents of a `topscript`. For example,
`sortedmulti_a` can only be placed iside a `tr(`, whereas `sortedmulti` can't
be placed inside a `tr(`.

## Descriptor keywords

A descriptor contains special keywords, such as "sh", "tr", "sortedmulti" etc.
These keywords are string literals, and are always followed by opening and
closing paranthesis, surrounding the text next to them. For example: "pk(0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798)".

When descibing the grammar, we will ommit the quotes for these keywords, to make
the notation less cumbersome. For example, instead of
`"sh(", token, ")"`, we will directly say `sh(token)`, where `token` could be
literal or a grammar rule.

## Script

The `topscript` is one of the following:
- `topscript` = sh(`subscript`) | sh(wsh(`subscript`)) | sh(wpkh(`key_no_uncompressed`)) | wsh(`subscript`) | wpkh(`key_no_uncompressed`) | combo(`key`), addr(`addr_spec`) | raw(`hex_str`) | rawtr(`key`)

The `subscript` is of two types:
- `subscript` = `script_key` | `multi_key`
- `script_key` = pk(`key`) | pkh(`key`)
- `multi_key` = multi(`n_keys` ',' `key_sequence`) | sortedmulti(`n_keys` ',' `key_sequence`)
- `n_keys` = `integer`
- `key_sequence` = `key` \[',' `key`\]\*

For multi_key, it's not mandatory that the amount of keys in the `key_sequence`
to match `n_keys`. This represents a k-of-n multisignature. So practically,
`n_keys` should be less than equal than the length of the `key_sequence`.

The `taproot_script` is a `tr` with either one or two arguments:
- `taproot_script` = tr((`key` | `key_public_hex_xonly`) \[',' tree\])

The `tree` has recursive structure, and represents a tree of keys:
- `tree` = (pk(`key` | `key_public_hex_xonly`]) | `multi_key_tr`) \['{' tree ',' tree '}'\]
- `multi_key_tr` = multi_a(`n_keys` ',' `key_sequence_xonly`) | sortedmulti_a(`n_keys` ',' `key_sequence_xonly`)
- `n_keys` = `integer`
- `key_sequence_xonly` = (`key` | `key_public_hex_xonly`) | \[',' (`key` | `key_public_hex_xonly`)\]\*

One particularity of taproot descriptors is that they allow x-only public keys i.e.
keys that contain the coordinate x of the curve, but don't contain the parity of y.

## Key

The `key` is the most complicated structure. That's because it can contain
encodings of other structures.

There are mainly 3 types of key specification:
- Hex encoded public keys
- WIF encoded private keys
- Extended private/public keys used for Hierarchical Deterministic wallets.

This is the grammar rule:
- `key` = [`key_origin`] (`key_public_hex` | `key_extended` | `key_private_wif`)
